...
export default function TopCategory() {
  ...
  const handleSubmit = async () => {
    ...
    if (error == false) {
      ...
      var responce = await postData("topcategories/submit_category", formData);
      if (responce.status) {
        Swal.fire({
          icon: "success",
          title: "Top Category",
    ...
    <div className={useStyle.root}>
      <div className={useStyle.box}>
        <Grid container spacing={3}>
          <Grid item xs={12}>
       <Heading image={categoryicon} caption="New Top Category" list="list"  link="/Display_all_Category" />
          </Grid>
          <Grid item xs={12}>
            <TextField
              value={CategoryName}
