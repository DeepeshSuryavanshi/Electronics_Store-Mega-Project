...
export default function Products(props) {
  ...
  const handleSubmit = async () => {
    ...
    if (error == false) {
      ...
      formData.append("file", Image.Bytes);
      console.log(formData)
      var responce = await postData("categorybanner/submit_categorybanner",formData);
      if (responce.status) {
        Swal.fire({
          icon: "success",
    ...
    <div className={useStyle.root}>
      <div className={useStyle.box}>
        <Grid container spacing={3}>
          <Grid></Grid>
          <Grid item xs={12}>
       <Heading image={categoryicon} caption=" Category banner" list="list"  link={"/Display_all_Products"} />
          </Grid>
         <Grid item xs={12} >
